/*
https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Math
*/

console.log(Math.E);
console.log(Math.PI);
// saknis is 2
console.log(Math.SQRT2);

// Math bibliotekos metodai
// metodai yra kaip funkcijos

// abs - absolut, absoliutine verte, be minuso zenklo
console.log('\n Abs ----------------');
console.log(Math.abs(7)); // -> 7
console.log(Math.abs(-7)); // -> 7
console.log(Math.abs(-3.14)); // -> 3.14
console.log(Math.abs(-Math.PI));
console.log(Math.abs(Math.PI));
console.log(Math.abs(3.567));

// istraukia kubine sakni - (2 * 2 * 2)
console.log('\n Cbrt ----------------');
console.log(Math.cbrt(8));
console.log(Math.cbrt(27));
console.log(Math.cbrt(64));
console.log(Math.cbrt(125));

// skaiciaus apvalinimas i virsu (3.4 -> 4)
console.log('\n Ceil ----------------');
console.log(Math.ceil(5));
console.log(Math.ceil(0));
console.log(Math.ceil(-5));
console.log(Math.ceil(3.14));
console.log(Math.ceil(-3.14));

// skaiciaus apvalinimas i mazesne puse (3.4 -> 3)
console.log('\n Floor ----------------');
console.log(Math.floor(5));
console.log(Math.floor(0));
console.log(Math.floor(-5));
console.log(Math.floor(3.14));
console.log(Math.floor(-3.14));

// matematinis skaiciaus apvalinimas
console.log('\n Round ----------------');
console.log(Math.round(5));
console.log(Math.round(0));
console.log(Math.round(-5));
console.log(Math.round(-3.14));

console.log(Math.round(3.00001));
console.log(Math.round(3.49999));
console.log(Math.round(3.5));
console.log(Math.round(3.50001));
console.log(Math.round(3.99999));

console.log(Math.round(-3.00001));
console.log(Math.round(-3.49999));
console.log(Math.round(-3.5));
console.log(Math.round(-3.50001));
console.log(Math.round(-3.99999));

// loginis apvalinimas
// pasiima skaiciaus sveikaja dali, meta lauk kas yra po kablelio
console.log('\n Trunc ----------------');
console.log(Math.trunc(5));
console.log(Math.trunc(0));
console.log(Math.trunc(-5));

console.log(Math.trunc(-3.00001));
console.log(Math.trunc(-3.49999));
console.log(Math.trunc(-3.5));
console.log(Math.trunc(-3.50001));
console.log(Math.trunc(-3.99999));

console.log(Math.trunc(7.00001));
console.log(Math.trunc(7.49999));
console.log(Math.trunc(7.5));
console.log(Math.trunc(7.50001));
console.log(Math.trunc(7.99999));

console.log('\n Exp ----------------');
console.log(Math.exp(17));
console.log(Math.E ** 17);

console.log('\n Fround ----------------');
console.log(Math.fround(5));
console.log(Math.fround(0));
console.log(Math.fround(-5));
console.log(Math.fround(3.1415));
console.log(Math.fround(-3.1415));
console.log(Math.fround(0.1));
console.log(Math.fround(-0.1));

// pitagoro teoremos principas
// priima kelis parametrus
console.log('\n Hypot ----------------');
console.log(Math.hypot(3, 4)); // 9 + 16 = 25 -> 5
console.log(Math.sqrt(3 ** 2 + 4 ** 2));
console.log(Math.hypot(1, 1, 1, 1));

console.log('\n Imul ----------------');
console.log(Math.imul(10, 17));

// grazina didziausia skaiciu
// neveikia array(masyvuose)
console.log('\n Max ----------------');
console.log(Math.max(1, 3));
console.log(Math.max(-1, -3));
console.log(Math.max(-1, -3, 7, 10, 3.14));

// grazina meziausia skaiciu
// neveikia masyvuose
console.log('\n Min ----------------');
console.log(Math.min(1, 3));
console.log(Math.min(-1, -3));
console.log(Math.min(-1, -3, 7, 10, 3.14));

const marks = [-1, -3, 7, 10, 3.14];
console.log(Math.min(marks), ':(');

// pakelia nurodytu laipsniu (2, 4 -> 2 ** 4 -> 16)
console.log('\n Pow ----------------');
console.log(Math.pow(2, 3));
console.log(Math.pow(3, 3));
console.log(2 ** 3);

console.log('\n Random ----------------');
console.log('Intervale nuo 0 iki 1 -> [0..1)');
console.log(Math.random());
console.log(Math.random());
console.log(Math.random());
console.log(Math.random());
console.log(Math.random());
console.log(Math.random());
console.log(0.9999999999999999);

console.log('\n Sign ----------------');
console.log(Math.sign(5));
console.log(Math.sign(-5));
console.log(Math.sign(-3.1415));
console.log(Math.sign(2.7178));
console.log(Math.sign(0));
console.log(Math.sign(-0));
console.log(Math.sign(-Infinity));
console.log(Math.sign(Infinity));
console.log(Math.sign(NaN));

console.log('\n Sqrt ----------------');
console.log(Math.sqrt(4));
console.log(Math.sqrt(9));
console.log(Math.sqrt(81));
console.log(Math.sqrt(-4), '2i');
console.log(Math.sqrt(-9), '3i');
console.log(Math.sqrt(-81), '9i');
// i*i = -1
